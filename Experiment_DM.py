...
from tuning import hyperopt_direct_method
...
if __name__ == '__main__':
    ...
    data_fp = Path('./data/')
    exp_name = 'DM1'   # diret method
    cv_index = 0    # 0-4

    x, y, a, _, d = load_bandit_data(fp=data_fp, fn='Bandit_1.pickle')
    ...
    data_list = [x_tr[tr_idx, :, :], a_tr[tr_idx, :], d_tr[tr_idx],
                 x_tr[val_idx, :, :], a_tr[val_idx, :], d_tr[val_idx],
                 x_te, y_te
                 ]
    ...
    func = partial(hyperopt_direct_method, data=data_list, counter=it, fp=temp_fp)

    config = {'repr_size': 16 * scope.int(hp.quniform('repr_size', 1, 8, 1)),
              'act_repr_size': 4 * scope.int(hp.quniform('act_repr_size', 1, 4, 1)),
