...
from utils import fix_random_seed, InvokeTimes, load_ebandit_data
...
if __name__ == '__main__':
    ...
    data_fp = Path('./data/')
    exp_name = 'eips1' #or etips1/2
    cv_index = 0    # 0-4

    # e(t)ips
    x, y, a, p_hat, d = load_ebandit_data(fp=data_fp, fn='eBandit_1.pickle')

    p_hat_clip = np.clip(p_hat, a_min=1.0/y.shape[1], a_max=None)
    ...
    p_hat_clip_tr = p_hat_clip[test_size:]
    ...
    print(f'shape of p_hat_clip_tr: {p_hat_clip_tr.shape}')
    ...
    data_list = [x_tr[tr_idx, :, :], a_tr[tr_idx, :], p_hat_clip_tr[tr_idx], d_tr[tr_idx],
                 x_tr[val_idx, :, :], a_tr[val_idx, :], p_hat_clip_tr[val_idx], d_tr[val_idx],
                 x_te, y_te
                 ]
